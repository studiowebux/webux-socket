 ...
 * @return {Function<Promise>} return callback with error or decoded user, and set the socket.user to the decoded user value.
 */
const checkAuth = (isAuthenticated, log = console) => {
   ...
   * @return {Promise} An error or a decoded user, and set the socket.user to the decoded user value.
   */
    ...
    return new Promise((resolve, reject) => {
      ...
      isAuthenticated(accessToken.toString(), (err, user) => {
        if (err || !user) {
          ...
          return reject(err || new Error("User not authenticated"));
        }
